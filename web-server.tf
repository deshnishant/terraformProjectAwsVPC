...
resource "aws_autoscaling_group" "my-web-asg" {
  name                 = "my-web-asg"
  launch_configuration = aws_launch_configuration.my-web-lconfig.id
  vpc_zone_identifier  = [aws_subnet.my-pub1-subnet.id, aws_subnet.my-pub2-subnet.id]
  min_size             = 2
  max_size             = 3
  desired_capacity     = 2
}
...
resource "aws_launch_configuration" "my-web-lconfig" {
  name_prefix                 = "my-web-lconfig"
  image_id                    = "ami-0b3a4110c36b9a5f0"
  instance_type               = "t2.micro"
  key_name                    = "web-asg-kp"
  security_groups             = [aws_security_group.my-ec2-asg-sg.id]
  user_data                   = <<-EOF
                                #!/bin/bash
